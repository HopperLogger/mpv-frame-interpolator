...
static bool blurFrameArray(struct OpticalFlowCalc* ofc, const bool outputBlurDirectly) {
    ...
    if (!ofc->frameBlurEnabled) {
        ERR_CHECK(cl_copy_buffer(ofc->queueOFC, ofc->inputFrameArray[0], 0, ofc->blurredFrameArray[0], 0,
                                 ofc->frameHeight * ofc->frameWidth * sizeof(unsigned short), "blurFrameArray"));
        return 0;
    }
    ...
    ERR_CHECK(cl_enqueue_kernel(ofc->queueOFC, ofc->blurFrameKernel, 2, ofc->grid16x16x1, ofc->threads16x16x1,
                                "blurFrameArray"));
    ...
    if (outputBlurDirectly) {
        ERR_CHECK(cl_copy_buffer(ofc->queueOFC, ofc->blurredFrameArray[0], 0, ofc->outputFrameArray, 0,
                                 ofc->frameHeight * ofc->frameWidth * sizeof(unsigned short), "blurFrameArray"));
        ERR_CHECK(cl_copy_buffer(
            ofc->queueOFC, ofc->inputFrameArray[0], ofc->frameHeight * ofc->frameWidth * sizeof(unsigned short),
            ofc->outputFrameArray, ofc->frameHeight * ofc->frameWidth * sizeof(unsigned short),
            0.5 * (ofc->frameHeight * ofc->frameWidth * sizeof(unsigned short)), "blurFrameArray"));
        ERR_CHECK(cl_finish_queue(ofc->queueOFC, "blurFrameArray"));
    }
    return 0;
}
...
bool downloadFrame(struct OpticalFlowCalc* ofc, const cl_mem sourceBuffer, unsigned char** outputPlanes) {
    ERR_CHECK(cl_download_buffer(ofc->queueWarping1, sourceBuffer, outputPlanes[0], 0,
                                 ofc->frameHeight * ofc->frameWidth * sizeof(unsigned short), "downloadFrame"));
    ERR_CHECK(cl_download_buffer(ofc->queueWarping1, sourceBuffer, outputPlanes[1],
                                 ofc->frameHeight * ofc->frameWidth * sizeof(unsigned short),
                                 (ofc->frameHeight >> 1) * ofc->frameWidth * sizeof(unsigned short), "downloadFrame"));
    return 0;
}
...
bool blurFlowArrays(struct OpticalFlowCalc* ofc) {
    ...
    if (!ofc->flowBlurEnabled) {
        ERR_CHECK(cl_copy_buffer(ofc->queueOFC, ofc->offsetArray12, 0, ofc->blurredOffsetArray12[1], 0,
                                 2 * ofc->opticalFlowFrameHeight * ofc->opticalFlowFrameWidth, "blurFlowArrays"));
...
bool warpFrames(struct OpticalFlowCalc* ofc, const float blendingScalar, const int frameOutputMode,
    ...
    if (isNewFrame) {
        ERR_CHECK(cl_copy_buffer(ofc->queueWarping1, ofc->inputFrameArray[0], 0, ofc->warpedFrameArray12, 0,
                                 3 * ofc->frameHeight * ofc->frameWidth, "warpFrames"));
    ...
    if (frameOutputMode != 1) {
        ...
        ERR_CHECK(cl_enqueue_kernel(ofc->queueWarping1, ofc->warpFrameKernel, 3, ofc->grid16x16x2, ofc->threads16x16x1,
                                    "warpFrames"));
    }
    ...
    if (frameOutputMode != 0) {
        ...
        ERR_MSG_CHECK(err, "warpFrames");
        ERR_CHECK(cl_enqueue_kernel(ofc->queueWarping2, ofc->warpFrameKernel, 3, ofc->grid16x16x2, ofc->threads16x16x1,
                                    "warpFrames"));
    }
...
bool blendFrames(struct OpticalFlowCalc* ofc, const float blendingScalar) {
    ...
    ERR_CHECK(cl_enqueue_kernel(ofc->queueWarping1, ofc->blendFrameKernel, 3, ofc->grid16x16x2, ofc->threads16x16x1,
                                "blendFrames"));
...
bool sideBySide1(struct OpticalFlowCalc* ofc) {
    ...
    ERR_CHECK(cl_enqueue_kernel(ofc->queueWarping1, ofc->sideBySide1Kernel, 3, ofc->halfGrid16x16x2,
...
bool sideBySide2(struct OpticalFlowCalc* ofc, const float blendingScalar, const int sourceFrameNum) {
    ...
    ERR_CHECK(cl_enqueue_kernel(ofc->queueWarping1, ofc->sideBySide2Kernel, 3, ofc->grid16x16x2, ofc->threads16x16x1,
                                "sideBySide2"));
...
bool visualizeFlow(struct OpticalFlowCalc* ofc, const int doBWOutput) {
    ...
    ERR_CHECK(cl_enqueue_kernel(ofc->queueWarping1, ofc->visualizeFlowKernel, 3, ofc->grid16x16x2, ofc->threads16x16x1,
                                "visualizeFlow"));
...
bool tearingTest(struct OpticalFlowCalc* ofc) {
    ...
    ERR_CHECK(cl_enqueue_kernel(ofc->queueWarping1, ofc->tearingTestKernel, 3, ofc->grid16x16x2, ofc->threads16x16x1,
                                "tearingTest"));
...
bool setKernelParameters(struct OpticalFlowCalc* ofc) {
    ...
    ERR_CHECK(cl_zero_buffer(ofc->queueOFC, ofc->offsetArray12,
                             numLayers * 2 * ofc->opticalFlowFrameHeight * ofc->opticalFlowFrameWidth, sizeof(char),
                             "reinit"));
    ERR_CHECK(cl_zero_buffer(ofc->queueOFC, ofc->offsetArray21,
                             2 * ofc->opticalFlowFrameHeight * ofc->opticalFlowFrameWidth, sizeof(char), "reinit"));
...
bool initOpticalFlowCalc(struct OpticalFlowCalc* ofc, const int frameHeight, const int frameWidth) {
    ...
    ERR_CHECK(cl_create_buffer(&ofc->inputFrameArray[2], ofc->clContext, CL_MEM_READ_ONLY, 3 * frameHeight * frameWidth,
                               "frame[2]"));
    ERR_CHECK(cl_create_buffer(&ofc->blurredFrameArray[0], ofc->clContext, CL_MEM_READ_WRITE,
                               frameHeight * frameWidth * sizeof(unsigned short), "blurredFrame[0]"));
    ERR_CHECK(cl_create_buffer(&ofc->blurredFrameArray[1], ofc->clContext, CL_MEM_READ_WRITE,
                               frameHeight * frameWidth * sizeof(unsigned short), "blurredFrame[1]"));
    ERR_CHECK(cl_create_buffer(&ofc->blurredFrameArray[2], ofc->clContext, CL_MEM_READ_WRITE,
                               frameHeight * frameWidth * sizeof(unsigned short), "blurredFrame[2]"));
    ...
    ERR_CHECK(cl_create_buffer(
        ...
        MAX_SEARCH_RADIUS * MAX_SEARCH_RADIUS * 2 * ofc->opticalFlowFrameHeight * ofc->opticalFlowFrameWidth,
        "offsetArray12"));
    ERR_CHECK(cl_create_buffer(&ofc->offsetArray21, ofc->clContext, CL_MEM_READ_WRITE,
                               2 * ofc->opticalFlowFrameHeight * ofc->opticalFlowFrameWidth, "offsetArray21"));
    ERR_CHECK(cl_create_buffer(&ofc->blurredOffsetArray12[0], ofc->clContext, CL_MEM_READ_WRITE,
                               2 * ofc->opticalFlowFrameHeight * ofc->opticalFlowFrameWidth,
    ...
    ERR_CHECK(cl_create_buffer(&ofc->summedDeltaValuesArray, ofc->clContext, CL_MEM_READ_WRITE,
                               MAX_SEARCH_RADIUS * MAX_SEARCH_RADIUS * ofc->opticalFlowFrameHeight *
                                   ofc->opticalFlowFrameWidth * sizeof(unsigned int),
                               "summedUpDeltaArray"));
