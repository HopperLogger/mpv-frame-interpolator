...
static void vf_HopperRender_process_intermediate_frame(struct mp_filter *f) {
    ...
    struct mp_image *img = mp_image_pool_get(priv->imagePool, IMG_FMT_SIZE == 2 ? IMGFMT_P010 : IMGFMT_NV12, priv->ofc->actualWidth, priv->ofc->frameHeight);
...
static struct mp_filter *vf_HopperRender_create(struct mp_filter *parent, void *options) {
    ...
    mp_autoconvert_add_imgfmt(priv->conv, IMG_FMT_SIZE == 2 ? IMGFMT_P010 : IMGFMT_NV12, 0);
