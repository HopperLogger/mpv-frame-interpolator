...
__kernel void sideBySide2Kernel(__global const unsigned char* sourceFrame,
                                __global const unsigned char* warpedFrame12,
                                __global const unsigned char* warpedFrame21, __global unsigned char* outputFrame,
    ...
    if (isYChannel && isInRightSideY) {
		// Y Channel
        blendedFrameValue = ((float)(warpedFrame12[((cy - verticalOffset) << 1) * dimX + ((cx - halfDimX) << 1)]) * frame1Scalar +
                             (float)(warpedFrame21[((cy - verticalOffset) << 1) * dimX + ((cx - halfDimX) << 1)]) * frame2Scalar);
    } else if (isUVChannel && isInRightSideUV) {
		// U/V Channels
        blendedFrameValue = ((float)(warpedFrame12[channelIndexOffset + 2 * (cy - (verticalOffset >> 1)) * dimX + ((cx - halfDimX) << 1) + isVChannel]) * frame1Scalar +
                             (float)(warpedFrame21[channelIndexOffset + 2 * (cy - (verticalOffset >> 1)) * dimX + ((cx - halfDimX) << 1) + isVChannel]) * frame2Scalar);
    }
    ...
    if (isYChannel) {
        if (isInLeftSideY) {
            ...
            outputFrame[cy * dimX + cx] =
                fmax(fmin(((float)(sourceFrame[((cy - verticalOffset) << 1) * dimX + (cx << 1)]) - outputBlackLevel) / (outputWhiteLevel - outputBlackLevel) * 255.0f, 255.0f), 0.0f);
        } else if (isInRightSideY) {
			// Y Channel Right Side
            outputFrame[cy * dimX + cx] = fmax(fmin(((float)blendedFrameValue - outputBlackLevel) / (outputWhiteLevel - outputBlackLevel) * 255.0f, 255.0f), 0.0f);
        } else {
        ...
        if (isInLeftSideUV) {
            ...
            outputFrame[dimY * dimX + cy * dimX + cx] = 
                fmax(fmin(((float)sourceFrame[channelIndexOffset + ((cy - (verticalOffset >> 1)) << 1) * dimX + (cx << 1) + isVChannel] - 128.0f) / outputWhiteLevel * 255.0f + 128.0f, 255.0f), 0.0f);
        } else if (isInRightSideUV) {
			// UV Channels Right Side
            outputFrame[dimY * dimX + cy * dimX + cx] = fmax(fmin((blendedFrameValue - 128.0f) / outputWhiteLevel * 255.0f + 128.0f, 255.0f), 0.0f);
        } else {
			// UV Channels Black Frame
            outputFrame[dimY * dimX + cy * dimX + cx] = 128;
        }
    }
}