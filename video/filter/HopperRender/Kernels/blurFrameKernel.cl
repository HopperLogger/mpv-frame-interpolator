...
__kernel void blurFrameKernel(__global const unsigned short* input, __global unsigned short* output, const int dimY,
                              const int dimX) {
    ...
    if (tx < KERNEL_RADIUS && ty < KERNEL_RADIUS) {
        ...
        int haloYBottom = gy + localSizeY;
        localTile[ty][tx] =
            (haloYTop >= 0 && haloXLeft >= 0) ? input[haloYTop * dimX + haloXLeft] : 0;  // Top Left square
        localTile[ty][tx + localSizeX + KERNEL_RADIUS] =
            (haloYTop >= 0 && haloXRight < dimX) ? input[haloYTop * dimX + haloXRight] : 0;  // Top Right square
        localTile[ty + localSizeY + KERNEL_RADIUS][tx] =
            (haloYBottom < dimY && haloXLeft >= 0) ? input[haloYBottom * dimX + haloXLeft] : 0;  // Bottom Left square
        localTile[ty + localSizeY + KERNEL_RADIUS][tx + localSizeX + KERNEL_RADIUS] =
            (haloYBottom < dimY && haloXRight < dimX) ? input[haloYBottom * dimX + haloXRight]
