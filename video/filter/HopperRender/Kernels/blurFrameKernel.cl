...
__kernel void blurFrameKernel(__global const unsigned short* input,
    ...
    if (ty < KERNEL_RADIUS) {
        ...
        localTile[ty][lx] = (haloYTop >= 0) ? input[haloYTop * width + gx] : 0;
        localTile[ty + localSizeY + KERNEL_RADIUS][lx] = (haloYBottom < height) ? input[haloYBottom * width + gx] : 0;
    }
    ...
    if (tx < KERNEL_RADIUS) {
        ...
        localTile[ly][tx] = (haloXLeft >= 0) ? input[gy * width + haloXLeft] : 0;
        localTile[ly][tx + localSizeX + KERNEL_RADIUS] = (haloXRight < width) ? input[gy * width + haloXRight] : 0;
    }
	...
	if (tx < KERNEL_RADIUS && ty < KERNEL_RADIUS) {
		...
		int haloYTop = gy - KERNEL_RADIUS;
		int haloYBottom = gy + localSizeY;
		localTile[ty][tx] = (haloYTop >= 0 && haloXLeft >= 0) ? input[haloYTop * width + haloXLeft] : 0; // Top Left square
		localTile[ty][tx + localSizeX + KERNEL_RADIUS] = (haloYTop >= 0 && haloXRight < width) ? input[haloYTop * width + haloXRight] : 0; // Top Right square
		localTile[ty + localSizeY + KERNEL_RADIUS][tx] = (haloYBottom < height && haloXLeft >= 0) ? input[haloYBottom * width + haloXLeft] : 0; // Bottom Left square
		localTile[ty + localSizeY + KERNEL_RADIUS][tx + localSizeX + KERNEL_RADIUS] = (haloYBottom < height && haloXRight < width) ? input[haloYBottom * width + haloXRight] : 0; // Bottom Right square
	}
