...
__kernel void warpFrameKernel(__global const unsigned char* sourceFrame12, __global const unsigned char* sourceFrame21,
    ...
    if (cz == 0 && cy < dimY && cx < dimX) {
        ...
        const int offsetX12 = offsetArray12[scaledCy * lowDimX + scaledCx];
        const int offsetY12 = offsetArray12[directionIndexOffset + scaledCy * lowDimX + scaledCx];
        const int offsetX21 = offsetArray21[scaledCy * lowDimX + scaledCx];
        const int offsetY21 = offsetArray21[directionIndexOffset + scaledCy * lowDimX + scaledCx];

        // Get the new pixel position
        const int newCx12 = mirrorCoordinate(adjCx + (int)round((float)(offsetX12) * frameScalar12), dimX);
        const int newCy12 = mirrorCoordinate(adjCy + (int)round((float)(offsetY12) * frameScalar12), dimY);
        const int newCx21 = mirrorCoordinate(adjCx + (int)round((float)(offsetX21) * frameScalar21), dimX);
        const int newCy21 = mirrorCoordinate(adjCy + (int)round((float)(offsetY21) * frameScalar21), dimY);
        ...
        // Get the new pixel position
        const int newCx12 = mirrorCoordinate(adjCx + (int)round((float)(offsetX12) * frameScalar12), dimX);
        const int newCy12 = mirrorCoordinate(adjCy + (int)round((float)(offsetY12) * frameScalar12), (dimY >> 1));
        const int newCx21 = mirrorCoordinate(adjCx + (int)round((float)(offsetX21) * frameScalar21), dimX);
        const int newCy21 = mirrorCoordinate(adjCy + (int)round((float)(offsetY21) * frameScalar21), (dimY >> 1));
